...
export function BookProductProvider({ children }: { children: React.ReactNode }) {
  ...
  const [fichesProduct, setFichesProduct] = useState<Product | null>(null)
  ...
  useEffect(() => {
    fetch('/api/book/pricing')
      ...
      .then((d) => {
        ...
        if (d?.fiches?.product) setFichesProduct(d.fiches.product)
      })
      .catch(() => {})
  ...
  return (
    <BookProductContext.Provider value={{ product, fichesProduct, loading }}>
      {children}
