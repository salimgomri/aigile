...
/**
 * Codes ISO acceptés par Stripe Checkout (shipping_address_collection).
 * Exclut les pays absents de l'API Stripe (ex. CU).
 */
const STRIPE_SHIPPING_COUNTRY_CODES = new Set<string>([
...
export function getStripeShippingCountryCodes(): Stripe.Checkout.SessionCreateParams.ShippingAddressCollection.AllowedCountry[] {
  const fromCatalog = COUNTRIES.map((c) => c.code).filter((code) =>
    STRIPE_SHIPPING_COUNTRY_CODES.has(code)
  )
  return fromCatalog as Stripe.Checkout.SessionCreateParams.ShippingAddressCollection.AllowedCountry[]
}
