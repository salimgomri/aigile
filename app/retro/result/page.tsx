      ...
      <div className="min-h-screen bg-gradient-to-br from-black via-slate-950 to-black relative overflow-hidden">
        ...
        <div className="relative z-10 w-full py-16 md:py-24 px-4 md:px-8">
          <div className="max-w-3xl mx-auto">
            <div className="bg-gradient-to-br from-white/[0.02] via-white/[0.01] to-transparent backdrop-blur-3xl rounded-[3rem] border border-white/10 p-8 md:p-12 shadow-2xl shadow-black/50">
                ...
                <div className="flex-1 pt-2">
                  <h1 className="text-4xl md:text-5xl font-semibold text-white leading-tight tracking-tight">
                    {unlockedPattern ? problemLabel : (
                      <button
                        type="button"
                        onClick={() => triggerGenerateRef.current?.()}
                        className="text-left hover:text-orange-400 transition-colors underline underline-offset-4 decoration-orange-500/50 hover:decoration-orange-400"
                      >
                        {problemLabel}
                      </button>
                    )}
              ...
              <p className="text-lg text-white/60 font-light leading-relaxed mb-6">
                {unlockedPattern ? problemDesc : (
                  <button
                    type="button"
                    onClick={() => triggerGenerateRef.current?.()}
                    className="text-left hover:text-orange-400/80 transition-colors underline underline-offset-4 decoration-orange-500/50 hover:decoration-orange-400"
                  >
                    {problemDesc}
                  </button>
                )}
                ...
                <div className="text-2xl font-semibold text-orange-400">
                  {unlockedPattern ? problemLabel : (
                    <button
                      type="button"
                      onClick={() => triggerGenerateRef.current?.()}
                      className="text-left hover:text-orange-300 transition-colors underline underline-offset-4 decoration-orange-500/50 hover:decoration-orange-400"
                    >
                      {problemLabel}
                    </button>
                  )}
              ...
              <CreditButton
                ...
                onConfirmed={async (res) => {
                  if (res?.pattern) setUnlockedPattern(res.pattern)
